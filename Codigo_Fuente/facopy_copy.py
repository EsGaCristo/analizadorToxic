...
def fa(contenido):
    ...
    TablaT=  [[0, '(', 1],[1,'a',2],[2, '>', 3],[3,'5',4],[4,')',5],[5,'{',6],[6,'}',7]]
    TablaW=  [[0, '(', 1],[1,'a',2],[2, '>', 3],[3,'5',4],[4,')',5],[5,'{',6],[6,'}',7]]
    TablaD=  [[0, 'a', 1],[1,'=',2],[2, 'b', 3],[3,';',4]]
    TablaAs= [[0, 'c', 1],[1,'=',2],[2, 'a', 3],[3,'+',4],[4,'b',5],[5,';',6]]
    TablaCam = [[0, 'call', 1],[1,'camara',2]]
    TablaMus = [[0, 'call', 1],[1,'musica',2]]
            ...
            while(i<cadTam):
                ...
                if caracter in Σ:
                    ...
                    if caracter == '{':
                        ...
                        subcadena = cadena[indices_c+1:indices_h]
    ...
    def casos(cadenaA):
        if re.findall("if",cadenaA):
                ...
                result = tablas(TablaT,TablaC,[7],EA,cadenaA)
                return result
        elif re.findall("whl",cadenaA):
                cadenaA = cadenaA.replace("whl","")
                cadenaA = cadenaA.replace("\n","")
                result =  tablas(TablaW,TablaC,[7],EA,cadenaA)
                return result
        elif re.findall("c=",cadenaA):
                #print('a')
                cadenaA = cadenaA.replace("\n","")
                cadenaA = cadenaA.replace(" ","")
                result = tablas(TablaAs,TablaC,[6],EA,cadenaA)
                return result
        elif re.findall("call",cadenaA):
            cadenaA = cadenaA.replace("call","")
            cadenaA = cadenaA.replace(" ","")
            result = tablas(TablaCam,TablaC,[2],EA,cadenaA)
            return result
        else:
            cadenaA = cadenaA.replace("\n","")
            cadenaA = cadenaA.replace(" ","")
            result = tablas(TablaD,TablaC,[4],EA,cadenaA)
            return result        
